...
to go
  ...
  ;; adjust belief according to our location
  ask turtles [
    if pcolor = green or pcolor = blue [ set belief belief + 5 ]
    if pcolor = black [ set belief belief - 1 ]
  ]

  ; affect others around us
  ask turtles with [ strong? ] [set belief belief + 1]
  ask turtles with [ negative? ] [set belief belief - 1]


  ; place limits on the degree of beliefs
  ask turtles with [ belief > 15 ] [ set belief 15 ]   ;; setting max beliefs
  ask turtles with [ belief < -5 ] [ set belief -5 ]     ;; setting minimum beliefs

  color-turtles

  ;; create propagators
  ask turtles with [ strong? ] [
...
to-report positive?
  report belief >= 5 and belief < 15
end

to-report neutral?
  report belief >= 0 and belief < 5
end
...
"" ""
PENS
"strong" 1.0 0 -14439633 true "" "plot count turtles with [ strong? ]"
"positive" 1.0 0 -11085214 true "" "plot count turtles with [ positive? ]"
"neutral" 1.0 0 -5509967 true "" "plot count turtles with [ neutral? ]"
"negative" 1.0 0 -7500403 true "" "plot count turtles with [ negative? ]"
...
"" ""
PENS
"belief" 1.0 0 -16777216 true "" "plot mean [ belief ] of turtles"
